...
const Blogs = () => {
    ...
    return (
        <section className="bg-gray-800 dark:bg-white text-gray-100 dark:text-gray-800 px-4 sm:px-8 lg:px-12 py-8">
            <div className="container max-w-6xl py-6 mx-auto space-y-6 sm:space-y-12">
                <Link to={`/blog/${blogs[0].id}`} rel="noopener noreferrer" className="block max-w-sm gap-3 mx-auto sm:max-w-full group hover:no-underline focus:no-underline lg:grid lg:grid-cols-12 bg-gray-900 dark:bg-gray-50">
                ...
                <div className="grid justify-center grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {
                        blogs.slice(1).map(blog => <BlogCard key={blog.id} data={blog} />)
                    }
                </div>
            </div>
        </section>
